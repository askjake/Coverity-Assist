...
.PHONY: stg-apply stg-proof stg-dns stg-all stg-bounce stg-harden \
        prod-apply prod-dns prod-all prod-harden \
        prod-canary-apply prod-canary-weight prod-canary-disable \
...
> if [[ -z "$$COVERITY_ASSIST_TOKEN" || -z "$$INFERENCE_PROFILE_ARN" ]]; then echo "Set COVERITY_ASSIST_TOKEN and INFERENCE_PROFILE_ARN in your shell"; exit 2; fi
...
> # Try a normal rollout wait, but don't die on transient churn
> if ! kubectl -n $(NS) rollout status deploy/coverity-assist --timeout=240s; then \
>   echo "rollout slow; last events:"; \
>   kubectl -n $(NS) get events --sort-by=.lastTimestamp | tail -n 40; \
> fi
> # Pod-level readiness (handles CrashLoopBackOff recoveries)
> kubectl -n $(NS) wait --for=condition=Ready --timeout=240s pod -l app=coverity-assist
> echo "Ready pods:" && kubectl -n $(NS) get pod -l app=coverity-assist -o wide

# -------- Stage proof (health/env/endpoints) --------
stg-proof:
> echo "=== Cluster-internal health ==="
> -kubectl -n $(NS) delete pod/ca-curl --ignore-not-found
> kubectl -n $(NS) run ca-curl --restart=Never --image=curlimages/curl -- sh -lc "curl -fsS http://coverity-assist.$(NS).svc.cluster.local:$(SVC_PORT)/health | tee /tmp/out; sleep 1"
...
>   kubectl -n $(NS) exec -c "$$CNAME" "$$POD" -- sh -lc 'python -c '"'"'import urllib.request,json; print(json.dumps(json.load(urllib.request.urlopen("http://127.0.0.1:'"$(PORT)"'/health")), indent=2))'"'"''
> kubectl -n $(NS) exec "$$POD" -c "$$CNAME" -- sh -lc 'echo -n TOKEN_LEN=; printenv COVERITY_ASSIST_TOKEN | wc -c; echo INFERENCE_PROFILE_ARN=$$INFERENCE_PROFILE_ARN'
...
>   | jq -r '{endpointIPs:(.items[0].endpoints|map(.addresses[0])), portName:(.items[0].ports[0].name), port:(.items[0].ports[0].port)}'
...
> if [[ -n "$$HN" ]]; then \
>   echo '{ "Comment": "CNAME for $(HOST)", "Changes": [ { "Action": "UPSERT", "ResourceRecordSet": { "Name": "$(HOST)", "Type": "CNAME", "TTL": 60, "ResourceRecords": [ { "Value": "'$$HN'" } ] } } ] }' > .change.json; \
> else \
>   echo '{ "Comment": "A for $(HOST)", "Changes": [ { "Action": "UPSERT", "ResourceRecordSet": { "Name": "$(HOST)", "Type": "A", "TTL": 60, "ResourceRecords": [ { "Value": "'$$IP'" } ] } } ] }' > .change.json; \
> fi; \
...
> i=0; until OUT=$$(curl -fsS "https://$(HOST)/health"); do i=$$((i+1)); if [ $$i -gt 30 ]; then echo "timeout waiting for external /health"; exit 1; fi; sleep 2; done; \
...
prod-harden:
> kubectl -n $(PROD_NS) apply -f k8s/hpa.yaml
> kubectl -n $(PROD_NS) apply -f k8s/pdb.yaml
> kubectl -n $(PROD_NS) apply -f k8s/netpol-egress.yaml
> kubectl -n $(PROD_NS) get hpa,pdb,netpol
...
# Weighted canary (0..100)
prod-canary-weight:
> if [[ -z "$$W" ]]; then echo "Usage: make prod-canary-weight W=20"; exit 2; fi
> kubectl -n $(PROD_NS) annotate ingress coverity-assist-canary-header nginx.ingress.kubernetes.io/canary-weight="$$W" --overwrite

# Disable weighted canary
prod-canary-disable:
> kubectl -n $(PROD_NS) annotate ingress coverity-assist-canary-header nginx.ingress.kubernetes.io/canary-weight- --overwrite
